import React from 'react';
import './Nav.css';
import {useState, useEffect } from 'react';



const Nav = ({onSearchHandler}) => {
 const [show, handleShow] = useState(false);
 const [movieSearch, setMovieSearch] = useState("");
    useEffect(() => {
        window.addEventListener("scroll",()=>{
            if(window.scrollY>100){
                handleShow(true);
            }
            else handleShow(false);
        });
        return()=>{
            window.removeEventListener("scroll");
        };
    }, []);
 
    const onChangeHandler=(e)=>{
        setMovieSearch(e.target.value);
    }

    return (
        <div className={`nav ${show && "nav_black"}`}>
            <img  className="nav_logo"  src="https://upload.wikimedia.org/wikipedia/commons/thumb/0/08/Netflix_2015_logo.svg/799px-Netflix_2015_logo.svg.png" alt="Netflix Logo"/>
            <img  className="nav_avatar" src="data:image/png;base64," alt="Netflix Avatar"/>
            
            <span className="nav_search">
            <input className="nav_searchbar" placeholder="Search" type="text" onChange={(e)=>onChangeHandler(e)} />
            <img className="nav_searchIcon" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqeQGOJZx2t2bxsbuHc2k1ADPBGZEk-yqEiQ&usqp=CAU" alt="search Icon" onClick={()=>onSearchHandler(movieSearch)} />
            </span>
            
        </div>
    )
}

export default Nav
